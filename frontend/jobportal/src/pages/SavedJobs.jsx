...
import { FiSearch, FiFilter, FiX, FiGrid, FiList, FiHeart, FiBookmark, FiTrash2, FiBriefcase, FiMapPin, FiClock } from "react-icons/fi";
...
const SavedJobs = () => {
    ...
    useEffect(() => {
        const loadSaved = async () => {
            try {
                ...
                // Extract jobs from saved items and mark as saved
                const savedJobs = res.data.map(item => ({
                    ...item.job,
                    is_saved: true,
                    saved_id: item.id, // Keep the saved item ID for unsaving
                }));
                setSaved(savedJobs);
                setPage(1);
            } catch (err) {
                ...
                setError("Could not load saved jobs. Please try again.");
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleUnsave = async (jobId, savedId) => {
        try {
            ...
            // TODO: Add API call to unsave job if you have an endpoint
            // Example: await unsaveJob(savedId);
            
            // Show temporary notification (you can add a toast system)
            console.log(`Unsaved job ${jobId}`);
        } catch (err) {
            console.error("Error unsaving job:", err);
            // Re-add to state if error
        ...
        <div style={pageContainerStyle}>
            ...
            <div style={contentContainerStyle}>
                ...
                <div style={headerStyle}>
                    <div>
                        <h1 style={titleStyle}>
                            <span style={{ color: '#4f46e5' }}>Saved</span>
                            <span style={{ color: '#7c3aed' }}> Jobs</span>
                        </h1>
                        <div style={subtitleContainerStyle}>
                            <span style={subtitleStyle}>
                                Your <span style={highlightTextStyle}>bookmarked</span> opportunities
                            </span>
                            <div style={trendingChipStyle}>
                                <div style={trendingDotStyle}></div>
                                <span>❤️ Come back and apply anytime</span>
                            </div>
                        </div>
                    </div>
                ...
                <div style={statsContainerStyle}>
                    <div style={statCardStyle}>
                        ...
                        <div>
                            <div style={statNumberStyle}>{saved.length}</div>
                    ...
                    <div style={statCardStyle}>
                        ...
                        <div>
                            <div style={statNumberStyle}>
                                {new Set(saved.map(job => job.job_type)).size}
                            </div>
                    ...
                    <div style={statCardStyle}>
                        ...
                        <div>
                            <div style={statNumberStyle}>
                                {new Set(saved.map(job => job.location)).size}
                            </div>
                    ...
                    <div style={statCardStyle}>
                        ...
                        <div>
                            <div style={statNumberStyle}>
                                {saved.filter(job => job.deadline).length}
                            </div>
                            <div style={statLabelStyle}>With Deadlines</div>
                        </div>
                    </div>
                </div>
                ...
                <div style={resultsHeaderStyle}>
                    <div style={resultsInfoStyle}>
                        <span style={resultsCountStyle}>
                            Showing <span style={highlightNumberStyle}>{Math.min(startIndex + 1, filteredJobs.length)}-{Math.min(startIndex + currentJobs.length, filteredJobs.length)}</span> of <span style={highlightNumberStyle}>{filteredJobs.length}</span> saved jobs
                        </span>
                        {activeFilterCount > 0 && (
                ...
                {loading && (
                    <div style={loadingContainerStyle}>
                        ...
                        <p style={loadingTextStyle}>
                            <span style={{ color: '#4f46e5' }}>Loading</span> your saved jobs...
                        </p>
                ...
                {error && !loading && (
                    <div style={errorContainerStyle}>
                        ...
                        <h4 style={errorTitleStyle}>Unable to load saved jobs</h4>
                        ...
                        {filteredJobs.length === 0 ? (
                            <div style={emptyStateStyle}>
                                ...
                                <h3 style={emptyTitleStyle}>
                                    {activeFilterCount > 0 ? "No matching saved jobs" : "No saved jobs yet"}
                                </h3>
                                <p style={emptyMessageStyle}>
                                    {activeFilterCount > 0 
                                        ? "Try adjusting your filters or search terms"
                                        : "Tap the heart icon ❤️ on any job to save it here for later."}
                                </p>
                                {activeFilterCount > 0 ? (
                                    <button 
                                    ...
                                    {currentJobs.map((job, index) => (
                                        ...
                                        >
                                            <JobCard 
                                                ...
                                                viewMode={viewMode}
                                                showSavedBadge={true}
                                                onUnsave={() => handleUnsave(job.id, job.saved_id)}
                                            />
                                        </div>
                                    ))}
                                </div>
